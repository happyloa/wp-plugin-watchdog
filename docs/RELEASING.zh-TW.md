# Site Add-on Watchdog 1.8.1 發布手冊
...
3. 建立安裝 ZIP：
   ...
   成品會是 `dist\site-add-on-watchdog-1.8.1.zip`，ZIP 內第一層必須是 `site-add-on-watchdog` 資料夾。
...
1. 將本機 `main` push 到 GitHub，但先不要建立 `1.8.1` tag。
...
1. 解壓縮 `dist\site-add-on-watchdog-1.8.1.zip`。
...
新檔案使用 **TortoiseSVN → Add**。選取 PNG 後開啟 **TortoiseSVN → Properties**，確認 `svn:mime-type` 是 `image/png`。這些檔案不可放入 `trunk\assets` 或 `tags\1.8.1\assets`。
...
3. 建議 commit 訊息：
   ...
   ```text
   Release 1.8.1: correct WPScan detection, Cron authentication, and release metadata
   ```

4. Commit 成功後記下 revision number。

## 七、用 TortoiseSVN 建立 1.8.1 tag

1. 在 `wp-svn\trunk` 上按右鍵，選 **TortoiseSVN → Branch/tag**。
2. `To URL` 填入同一個 SVN repository 下的 `/tags/1.8.1`。
...
5. 按 OK 建立 server-side copy。完成後可在 Repo-browser 確認 `tags/1.8.1` 存在。
6. 已發布的 tag 視為不可變；若發現問題，回到 trunk 修正並發布 `1.8.2`，不要直接改 `tags/1.8.1`。

## 八、WordPress.org 發布確認

1. WordPress.org 偵測到新 tag 後，若此外掛有啟用 Release Confirmation，開啟通知信中的連結或外掛頁面的 Release Management 提示。
2. 確認 `1.8.1` 發布。沒有完成這一步時，tag 可能已存在但使用者還收不到更新。
3. 等待 CDN 與目錄快取更新；圖示和 banner 通常幾分鐘，尖峰時可能數小時。
4. 驗證外掛頁顯示版本 1.8.1、Tested up to 7.0、新 changelog、圖示與 banner。
...
1. 在通過 CI 的同一個 commit 建立 annotated tag：
   ...
   ```powershell
   git tag -a 1.8.1 -m "Site Add-on Watchdog 1.8.1"
   git push origin 1.8.1
   ```

2. 在 GitHub 建立 `1.8.1` Release，標題使用 `Site Add-on Watchdog 1.8.1`。
3. Release notes 使用 `CHANGELOG.md` 的 1.8.1 內容。
4. 上傳 `dist\site-add-on-watchdog-1.8.1.zip`，再發布 Release。

## 十、發布後快速驗證與回復策略

1. 從 WordPress.org 或 GitHub Release 重新下載公開 ZIP，不要使用本機來源檔代替。
2. 在測試站從 1.8.0 更新到 1.8.1，確認設定、忽略清單、歷史、cron secret 與通知設定都有保留。
3. 執行手動掃描，檢查後台、WP-Cron、REST cron 與通知佇列。
4. 若只是不影響網站的文件或目錄資產問題，可修正 trunk/assets 後重新 commit；不要改已發布 tag。
5. 若程式有回歸，立即在 Git 與 SVN trunk 修正、把版本升為 1.8.2、跑完同一套測試並建立新 tag。不要刪除或覆寫 1.8.1 tag。
